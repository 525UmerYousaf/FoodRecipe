...
const FoodCard = ({ item, index, navigation }) => {
  return (
    <View
      style={[styles.cardContainer, { paddingLeft: 20, paddingRight: 15}]} testID="foodDisplay"
    >
      <TouchableOpacity onPress={() => navigation.navigate("RecipeDetail", { ...item })}>
        ...
        <Text style={styles.articleText}>{item.recipeName.length > 20 ? item.recipeName.slice(0, 20) + "..." : item.recipeName}</Text>
        <Text style={styles.articleDescription}>{item.cookingDescription.length > 40 ? item.cookingDescription.slice(0, 40) + "..." : item.cookingDescription}</Text>
      </TouchableOpacity>
    </View>
  );
};
